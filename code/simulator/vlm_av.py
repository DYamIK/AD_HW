...
class OpenAICompatibleVLMLivePolicy:
    ...
    def _build_messages(
        ...
        system = (
            "你是 CARLA 自动驾驶换道决策模型，这是一条4车道场景，输入包含前视图(车头视角，放在你车的正前方车头上，所以你前方的车就是别的车，不是你自己的车，请注意区分）、鸟瞰图（从上往下看，图中间是你的车，上下左右是其他车辆，请注意区分）。\n"
            "你必须仅输出一个 JSON 对象，字段：\n"
            "- dx: float，0.0\n"
        ...
        user_text = (
            f"当前速度 speed_mps={speed_mps:.2f}，当前车道索引 lane_index={lane}（右=-1，中=0，左=+1）。目前4个车道，最右边的车道为不可通行车道\n"
            f"{memory_text}"
